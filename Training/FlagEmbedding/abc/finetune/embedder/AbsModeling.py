        ...
        if self.training:
            ...
            if self.use_mrl:
                ...
                for dim in self.mrl_dims:
                    # Extract embeddings up to current dimension
                    q_reps_dim = q_reps[..., :dim]
                    p_reps_dim = p_reps[..., :dim]
                    scores_dim, dim_loss = compute_loss_func(q_reps_dim, p_reps_dim, teacher_targets=teacher_targets)
                    scores_dict[f'scores_dim_{dim}'] = scores_dim
                    total_loss += dim_loss
                loss = total_loss / len(self.mrl_dims)
                scores = scores_dict  # Return all dimension scores
            else:
